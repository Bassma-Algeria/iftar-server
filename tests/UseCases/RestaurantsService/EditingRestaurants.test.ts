...
describe('Editing Restaurants', () => {
    ...
    it('only the owner of the restaurant can edit it', async () => {
        ...
        const restaurant = await restaurantsService.registerRestaurant({
            authToken,
            restaurantInfo,
        });
    ...
    it('should not be able to edit a non existing restaurant', async () => {
        await expect(
            restaurantsService.updateRestaurant({ authToken, newRestaurantInfo: restaurantInfo }),
        ).to.be.rejected;
    });

    it('should update the restaurant info when everything is ok', async () => {
        const restaurant = await restaurantsService.registerRestaurant({
            authToken,
            restaurantInfo,
        });
        ...
        const { ownerNumber, ...updatedInfo } = await restaurantsService.getRestaurantById(
            restaurant.restaurantId,
        );

        expect(updatedInfo).to.deep.equal(newRestaurantInfo);
    });

    it('should not have the old restaurant pictures when no one of them exist in the updated restaurant information', async () => {
        const restaurant = await restaurantsService.registerRestaurant({
            authToken,
            restaurantInfo,
        });
        ...
        await restaurantsService.updateRestaurant({ authToken, newRestaurantInfo });
        const updatedInfo = await restaurantsService.getRestaurantById(restaurant.restaurantId);

        expect(updatedInfo?.pictures).be.an.empty('array');
    });
    ...
    it('should merge the picture when adding new once and keeping some of the old ones', async () => {
        ...
        expect(updatedInfo?.pictures).be.an('array').and.have.lengthOf(4);
        expect(updatedInfo?.pictures).to.not.deep.equal(restaurant.pictures);
        expect(updatedInfo?.pictures).to.include(restaurant.pictures[0]);
        expect(updatedInfo?.pictures).to.include(restaurant.pictures[1]);
    });

    it("should delete the pictures we don't need anymore from the cloud", async () => {
