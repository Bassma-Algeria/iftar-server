...
describe('Getting Restaurants', () => {
    ...
    it('should get the restaurant information by its id + the restaurant owner phone number number', async () => {
    ...
    it('should return an empty array when no restaurants exit when getting all the restaurants', async () => {
        await expect(restaurantsService.getAllRestaurants()).to.eventually.be.empty;
    });

    it('should return not be able to get a non existing restaurant', async () => {
        await expect(restaurantsService.getRestaurantById('not exist')).to.be.rejected;
    });
    ...
    it("should return an empty array when the owner didn't register any restaurant", async () => {
        await expect(restaurantsService.getRestaurantsOfAuthOwner(authToken)).to.eventually.be
            .empty;
    });
